...
export default function DatasetPageClient({ data }) {
  ...
  const [showDataDictModal, setShowDataDictModal] = useState(false);
  ...
  useEffect(() => {
    ...
    fetch(data.xsd)
      ...
      .then(content => setXmlContent(content))
  ...
  const loadExcelPreview = async () => {
    ...
    try {
      const response = await fetch(data.datadict);
  ...
  const handleDataDictClick = (e) => {
    e.preventDefault();
    setShowDataDictModal(true);
    loadExcelPreview();
  };
    ...
    <div className="bg-linear-to-br from-slate-50 to-blue-50">
          ...
          <div className="bg-slate-900 rounded-2xl shadow-2xl overflow-hidden border border-slate-700">
            ...
            <div className="h-96 overflow-auto p-6">
              <pre className="text-slate-300 text-sm font-mono leading-relaxed whitespace-pre-wrap">
                {xmlContent || "กำลังโหลด XML Schema..."}
              </pre>
            </div>
          </div>
        </div>
      ...
      {showDataDictModal && (
        <div 
          ...
          onClick={() => setShowDataDictModal(false)}
        >
          <div 
